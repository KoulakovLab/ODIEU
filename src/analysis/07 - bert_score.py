...
for candidate_name, candidate in tqdm(candidates.items()):
    ...
    metrics_results[candidate_name] = bertscore.compute(predictions=cand, references=refs, lang="en")
# Aggregate
nlp_metrics_results = pd.DataFrame.from_dict(metrics_results, orient="tight")
nlp_metrics_results.to_csv("nlp_metrics_results_bertscore_0.csv")
pd.to_pickle(nlp_metrics_results, "nlp_metrics_results_bertscore_0.pkl")
scipy.io.savemat("nlp_metrics_results_bertscore_0.mat", nlp_metrics_results)
...
for b in tqdm(range(1, 10)):
    ...
    nlp_metrics_results.to_csv(f"nlp_metrics_results_bertscore_{b}.csv")
    pd.to_pickle(nlp_metrics_results, f"nlp_metrics_results_bertscore_{b}.pkl")
    scipy.io.savemat(f"nlp_metrics_results_bertscore_{b}.mat", nlp_metrics_results)
